...
CREATE TABLE `t_user_position` (
  `id` bigint(20) NOT NULL AUTO_INCREMENT,
  `user_id` bigint(20) NOT NULL COMMENT '用户id',
  `category_id` bigint(20) NOT NULL COMMENT '职位类别ID',
  `position` varchar(50) NOT NULL COMMENT '职位名称(冗余)',
  PRIMARY KEY (`id`),
  UNIQUE KEY `user_id_category_id` (`user_id`,`category_id`)
) ENGINE=MyISAM AUTO_INCREMENT=6 DEFAULT CHARSET=utf8 COMMENT='用户职位信息表';
